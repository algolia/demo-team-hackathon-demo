    ...
    <div className="relative self-center" ref={ref}>
      <button
        ...
        className={`code-btn z-20 relative rounded w-[20px] flex justify-center h-[20px] items-center transition-all ease-in-out ${
          isComponentVisible
            ? `bg-[#002451]  self-end ${openDirection === 'upLeft' ? 'rounded-tr-none rounded-tl-none' : 'rounded-br-none rounded-bl-none'}`
      ...
      <div
        className={`bg-[#002451] rounded-md shadow-xl p-4 z-[100] absolute  ${isComponentVisible ? 'block' : 'hidden'} ${
          openDirection === 'left' ? 'right-0 rounded-tr-none top-full' : ''
        } ${openDirection === 'right' ? 'left-0 rounded-tl-none top-full' : ''} ${openDirection === 'center' ? '-ml-[254px] ' : ''} ${
          openDirection === 'upLeft' ? 'right-0 bottom-full rounded-br-none' : ''
        }`}
      >
